                ...
                <div class="cart-group divide pr" :data-productid="item.productId">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <span class="blue-checkbox-new " :class="{'checkbox-on':(item.checked == true || item.checked == 'true')}"
    ...
    computed: {
      ...
      checkedCount () {
        ...
        this.cartList && this.cartList.forEach((item) => {
          if (item.checked === true || item.checked === 'true') i++
        })
        return Number(i)
      },
      // 计算总数量
    ...
    methods: {
      ...
      editCheckAll () {
        ...
        editCheckAll({userId: this.userId, checked: checkAll}).then(res => {
          _this.EDIT_CART({checked: checkAll})
        })
        // this.EDIT_CART({checked: checkAll})
        // this.cartList.forEach(item => {
        //   this._cartEdit(this.userId, item.productId, item.productNum, item.checked)
        // })
      },
      // 修改购物车
      ...
      editCart (type, item) {
        if (type && item) {
          ...
          // 勾选
          if (type === 'check') {
            let newChecked = (checked === true || checked === 'true') ? false : true
      ...
      delChecked () {
        var _delCartChecked = delCartChecked
        getCartList().then(res => {
          if (res.success === true) {
            ...
            _delCartChecked({userId: this.userId}).then(res => {
              if (res.success !== true) {
                this.message('删除失败')
              }
            })
