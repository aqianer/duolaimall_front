          ...
          <div class="box-inner ui-cart">
            <div>
              ...
              <div class="cart-table">
                <div class="cart-group divide pr" :data-productid="product.productId">
                    ...
                    <div class="cart-items clearfix">
                      ...
                      <div class="items-thumb fl">
                        <img :alt="product.productName"
                             :src="product.big">
                        <a @click="goodsDetails(product.productId)" :title="product.productName" target="_blank"></a>
                      </div>
                      <!--信息-->
                      <div class="name hide-row fl">
                        <div class="name-table">
                          <a @click="goodsDetails(product.productId)" :title="product.productName" target="_blank"
                             v-text="product.productName"></a>
                          <!-- <ul class="attribute">
                            <li>白色</li>
                          </ul> -->
                        </div>
                      ...
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px">¥ {{product.promoPrice * product.productNum}}</div>
                        <!--数量-->
                        <div class="item-cols-num">
                          <span v-text="product.productNum"></span>
                        </div>
                        <!--价格-->
                        <div class="price">¥ {{(product.promoPrice) * product.productNum}}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="shipping">
                  <div class="shipping-box" style="padding: 0 40px;">
                    <div class="shipping-total shipping-price"><h4
                      class="highlight">应付总额：<em>￥{{(product.promoPrice) * product.productNum}}</em>
                    </h4>
                    </div>
                  </div>
    ...
    methods: {
      ...
      _submitOrder () {
        ...
        seckill(params).then(res => {
          ...
          if (res.code === 200) { // 秒杀成功
            _this.$router.push({path: '/order/orderSeckillsuccess'})
          } else { // 库存不够, 失败
            alert(res.message)
            setTimeout(() => {
          ...
          _this.panelContentItems1 = res.result.productList
        }).catch(error => {
          alert('秒杀失败')
          _this.$router.push({path: '/'})
          console.log(error)
