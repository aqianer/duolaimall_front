...
<script>
  ...
  import { queryStatus } from '/api/goods'
  export default {
    data () {
      return {
        show: true,
    ...
    methods: {
      ...
      payCheck (orderId) {
        ...
        let timer = setInterval(() => {
          // 判断访问次数, 36次(三分钟)没有支付就停止查询
          _this.payCheckTimes++
          if (_this.payCheckTimes > 36) {
            clearInterval(timer)
            alert('订单超时,支付失败')
            setTimeout(() => {
              // TODO 跳转到首页
    ...
    mounted () {
      // let qrcodeURL = getStore('qrcode')
      let qrcodeURL = this.$route.query.qrcode
      ...
      if (isCustom !== 'true') {
        ...
        this.imgPath = qrcodeURL
        setTimeout(() => { this.payCheck(orderId) }, 5000)
      }
    },
    destroyed () {
