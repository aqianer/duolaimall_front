    ...
    <y-shelf v-bind:title="orderTitle">
      <div slot="content">
        <div v-loading="loading" element-loading-text="加载中..." style="min-height: 10vw;" v-if="orderList.length">
          <div class="orderStatus" v-if="orderStatus !== 5 && orderStatus !== 6 && orderStatus !== 7">
          ...
          <div class="status-now" v-if="orderStatus === 5" style="margin-top: 10px">
            <ul>
          ...
          <div class="status-now" v-if="orderStatus === 6" style="margin-top: 10px">
            <ul>
              <li class="status-title"><h3>订单状态：创建失败</h3></li>
            </ul>
            <p class="realtime">
              <span>您的订单创建失败。</span>
            </p>
          </div>
          <div class="status-now" v-if="orderStatus === 7" style="margin-top: 10px">
            <ul>
      ...
      _getOrderDet () {
        ...
        getOrderDet(params).then(res => {
          ...
          if (res.result.orderStatus === 0) {
            _this.orderStatus = 1
          } else if (res.result.orderStatus === 1) { // 已支付
            _this.orderStatus = 2
          } else if (res.result.orderStatus === 4) { // 交易成功
            _this.orderStatus = 3
          } else if (res.result.orderStatus === 5) { // 交易关闭
            _this.orderStatus = 5
          } else if (res.result.orderStatus === 6) { // 交易失败
            _this.orderStatus = 6
          } else if (res.result.orderStatus === 7) { // 交易取消
            _this.orderStatus = 7
          }
          _this.orderList = res.result.goodsList
          _this.orderTotal = res.result.orderTotal
          _this.userName = res.result.userName
          _this.tel = res.result.tel
          _this.streetName = res.result.streetName
          _this.createTime = res.result.createDate
          _this.closeTime = res.result.closeDate
          _this.payTime = res.result.payDate
          if (_this.orderStatus === 3) {
            _this.finishTime = res.result.finishDate
          } else {
            _this.countTime = res.result.finishDate
          }
          _this.loading = false
          // console.log(this.orderStatus + '**1')
          // console.log(_this.orderStatus + '**2')
        })
